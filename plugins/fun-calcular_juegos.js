let handler = async (m, { conn, command, text, usedPrefix }) => { 
 ...
  // ------------------------------------------------------------------------------------------------------------------------------------------------    
   if (command == 'رجال') { 
 let juego = `_*${text.toUpperCase()} رجوله بنسبا ${(500).getRandom()}%*_`.trim() 
 await conn.reply(m.chat, juego, m, m.mentionedJid ? { mentions: m.mentionedJid } : {})} 
   // ------------------------------------------------------------------------------------------------------------------------------------------------ 
    
 if (command == 'غباء') { 
 ...
 handler.help = ['حب', 'جمال', 'lesbiana', 'غباء', 'pajera', 'puto', 'puta', 'ذكاء', 'رجال', 'rata', 'prostituta', 'prostituto'].map(v => v + ' @tag | nombre') 
 handler.tags = ['calculator'] 
 handler.command = /^حب|جمال|lesbiana|غباء|pajera|puto|puta|ذكاء|رجال|rata|prostituta|prostituto/i 
 handler.exp = 100 
